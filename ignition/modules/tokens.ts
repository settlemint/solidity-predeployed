...
export const TokensModule = buildModule("TokensModule", (m) => {
  ...
  const createUSDC = m.call(tokenFactory, "createToken", ["USD Coin", "USDC"], {
    id: "createUSDC",
  });
  const usdcAddress = m.readEventArgument(
    createUSDC,
    "TokenCreated",
    "tokenAddress",
    { id: "readUSDCAddress" }
  ...
  m.call(usdc, "mint", [deployer, "1000000000000000000000000000"], {
    id: "mintUSDC",
  });

  // USDT (18 decimals) - 1 million USDT
  const createUSDT = m.call(
    tokenFactory,
    "createToken",
    ["Tether USD", "USDT"],
    { id: "createUSDT" }
  );
  const usdtAddress = m.readEventArgument(
    createUSDT,
    "TokenCreated",
    "tokenAddress",
    { id: "readUSDTAddress" }
  ...
  m.call(usdt, "mint", [deployer, "1000000000000000000000000000"], {
    id: "mintUSDT",
  });

  // DAI (18 decimals) - 1 million DAI
  const createDAI = m.call(
    tokenFactory,
    "createToken",
    ["Dai Stablecoin", "DAI"],
    { id: "createDAI" }
  );
  const daiAddress = m.readEventArgument(
    createDAI,
    "TokenCreated",
    "tokenAddress",
    { id: "readDAIAddress" }
  ...
  m.call(dai, "mint", [deployer, "1000000000000000000000000000"], {
    id: "mintDAI",
  });

  // BOND (18 decimals) - 100,000 BOND
  const createBond = m.call(
    tokenFactory,
    "createToken",
  ...
  m.call(bond, "mint", [deployer, "1000000000000000000000000000"], {
    id: "mintBond",
  });

  // CALL (18 decimals) - 100,000 CALL
  const createOption = m.call(
    tokenFactory,
    "createToken",
    ["ETH Call Option", "CALL"],
    { id: "createOption" }
  );
  const optionAddress = m.readEventArgument(
    createOption,
    "TokenCreated",
    "tokenAddress",
  ...
  m.call(option, "mint", [deployer, "1000000000000000000000000000"], {
    id: "mintOption",
  });

  // BTCF (18 decimals) - 10,000 BTCF
  const createFuture = m.call(
    tokenFactory,
    "createToken",
    ["BTC Future", "BTCF"],
    { id: "createFuture" }
  );
  const futureAddress = m.readEventArgument(
    createFuture,
    "TokenCreated",
    "tokenAddress",
  ...
  m.call(future, "mint", [deployer, "1000000000000000000000000000"], {
    id: "mintFuture",
  });

  // SWAP (18 decimals) - 100,000 SWAP
  const createSwap = m.call(
    tokenFactory,
    "createToken",
    ["Interest Rate Swap", "SWAP"],
    { id: "createSwap" }
  );
  const swapAddress = m.readEventArgument(
    createSwap,
    "TokenCreated",
    "tokenAddress",
    { id: "readSwapAddress" }
  ...
  m.call(swap, "mint", [deployer, "1000000000000000000000000000"], {
    id: "mintSwap",
  });

  // XAUT (18 decimals) - 50,000 XAUT
  const createSynthetic = m.call(
    tokenFactory,
    "createToken",
    ["Synthetic Gold", "XAUT"],
    { id: "createSynthetic" }
  );
  const syntheticAddress = m.readEventArgument(
    createSynthetic,
    "TokenCreated",
    "tokenAddress",
    { id: "readSyntheticAddress" }
  );
  const synthetic = m.contractAt("Token", syntheticAddress, {
    id: "contractSynthetic",
  });
  m.call(synthetic, "mint", [deployer, "50000000000000000000000000"], {
    id: "mintSynthetic",
  });

  // Get the test accounts (Anvil provides 10 by default, we'll use 1-9)
  const testAccounts = Array.from(
    { length: 9 },
    (_, i) => m.getAccount(i + 1) // Skip 0 as it's the deployer
  );
  ...
  testAccounts.forEach((address, i) => {
    m.call(usdc, "mint", [address, amounts[i]], { id: `mintUSDC${i + 1}` });
  });
